...
class MusicScoreDataset(Dataset):
    def __init__(self, image_dir, transform=None, vocab=None, max_seq_len=60, num_samples=None):
...
def ctc_decode(logits, idx_to_token, blank=0):
    logits = logits.permute(1, 0, 2)  # (seq_len, batch, vocab_size)
    preds = torch.argmax(logits, dim=2).squeeze(1)  # (seq_len,)
    # Debug: Print raw predictions
    print("Raw prediction indices:", preds.tolist())
    decoded = []
    prev = None
    for idx in preds:
        idx = idx.item()
...
if __name__ == "__main__":
    ...
    train_image_dir = "/Users/leosvjetlicic/Desktop/Diplomski/primusCalvoRizoAppliedSciences2018/package_aa"  # Contains subdirs with .png and .semantic
    val_image_dir = "/Users/leosvjetlicic/Desktop/Diplomski/primusCalvoRizoAppliedSciences2018/package_ab"    # Contains subdirs with .png and .semantic

    # Load datasets
    # Combine datasets and split
    train_dataset = MusicScoreDataset(train_image_dir, transform=None, num_samples=None)
    val_dataset = MusicScoreDataset(val_image_dir, transform=None, vocab=train_dataset.vocab, num_samples=None)
